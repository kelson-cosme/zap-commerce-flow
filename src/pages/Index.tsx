...
const Index = () => {
  ...
  // --- EFEITO DE TEMPO REAL CORRIGIDO ---
  useEffect(() => {
    ...
    const conversationsChannel = supabase
      ...
      async (payload) => {
          if (payload.eventType === 'INSERT') {
            // Se for uma NOVA conversa, busca os dados completos e adiciona à lista
            ...
            // Se for uma ATUALIZAÇÃO, apenas atualiza a conversa existente
  ...
  const handleSendMessage = async (text: string) => {
    if (!selectedCustomer || !selectedConversationId || !session?.user?.id) return;
    
    const conv = whatsappConversations.find(c => c.id === selectedConversationId);
    if (!conv || !conv.contact?.phone_number) return;

    // 1. Verifica se a conversa está na fila (não atribuída)
    if (conv.assigned_to === null) {
      console.log(`Atribuindo conversa ${selectedConversationId} ao agente ${session.user.id}...`);
      
      // Atribui a conversa a si mesmo
      const { error } = await supabase.rpc('assign_conversation', { 
          conv_id: selectedConversationId,
          agent_id: session.user.id
      });

      if (error) {
          toast({ title: "Erro", description: "Não foi possível atender à conversa.", variant: "destructive" });
          return;
      }

      // Atualiza o estado local para mover a conversa para "Meus Atendimentos"
      setWhatsappConversations(currentConvs => 
          currentConvs.map(c => 
              c.id === selectedConversationId ? { ...c, assigned_to: session.user.id } : c
          )
      );
    }
    
    // 2. Envia a mensagem (como já fazia)
    try {
      await sendMessage(conv.contact.phone_number, text);
    } catch (error) {
  ...
  const handleLogout = async () => { await supabase.auth.signOut(); };
    ...
    if (error) {
        toast({ title: "Erro", description: "Não foi possível fechar a conversa.", variant: "destructive" });
        return;
    }

    // Remove a conversa da lista local para a UI atualizar instantaneamente
    setWhatsappConversations(currentConvs => 
        currentConvs.filter(c => c.id !== selectedConversationId)
    );

    // Limpa a seleção do chat
    setSelectedConversationId(null);
    setSelectedCustomerId(null);
